...
def event_definition(cls):
    ...
    def new_init(self, *args, **kwargs):
        # first call any existing __init__
        if original_init is not None:
            original_init(self, *args, **kwargs)

        # now grab *evaluated* annotations
        hints = get_type_hints(cls)

        for attr_name, attr_type in hints.items():
            default_event = getattr(cls, attr_name, None)

            # if you provided a default ConditionEvent in the class, copy its flags
            if isinstance(default_event, ConditionEvent):
                new_event = ConditionEvent(flags=default_event._parameters_def,
                                           id=attr_name)
                setattr(self, attr_name, new_event)

            # otherwise if the annotation *is* ConditionEvent, just make a fresh one
            elif attr_type is ConditionEvent:
                setattr(self, attr_name, ConditionEvent(id=attr_name))
