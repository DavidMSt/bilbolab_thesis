...
class Event:
    ...
    def on(self,
        ...
        # This is a special case, and we do not need a listener,
...
class EventWaiter:
    ...
    def __init__(self,
        ...
        self.events_finished = [False for _ in range(len(events))]

        self.matched_payloads: list[tuple[dict[str, Any] | None, Any | None]] = [
            (None, None) for _ in range(len(self.events))]
    ...
    def wait(self) -> bool:
        try:
            ...
            ret = self._signal.wait(timeout=self.timeout)
            return not self._abort and ret
        finally:
            # Clean up if we timed out or were stopped without being removed by the loop
            if self in active_event_loop.waiters:
    ...
    # ------------------------------------------------------------------------------------------------------------------
    def set_match(self, pos: int, flags: dict[str, Any], data: Any):
...
class EventListener:
    ...
    thread: threading.Thread  # Thread that runs the callback in

    max_rate: float = None  # Max rate in Hz for the callback to be executed
    input_data: bool
    _exit: bool = False
    ...
    def __init__(self, event: Event | list[Event],
                 predicate: Predicate | list[None | Predicate] = None,
        ...
        self.max_rate = max_rate  # TODO: How to implement this?
        self.wait_for_all = wait_for_all
        self.once = once
    ...
    def _task(self):
        while not self._exit:
            self._waiter = EventWaiter(events=self.events,
                                       predicates=self.predicate,
                                       wait_for_all=self.wait_for_all,
                                       stale_event_time=None,
                                       timeout=None)
            success = self._waiter.wait()
            if not success:
                break

            if len(self.events) == 1:
                # prefer matched snapshot over any shared state
            ...
            # run callback (as you already do)
            if self.spawn_thread:
                th = threading.Thread(target=self.callback, args=(data,), daemon=True) if self.input_data \
                    else threading.Thread(target=self.callback, daemon=True)
                th.start()
            else:
                self.callback(data) if self.input_data else self.callback()
...
# === FUNCTIONS ========================================================================================================
def waitForEvents(events: Event | list[Event],
                  predicates: Predicate | list[Predicate] = None,
                  timeout=None, wait_for_all=False) -> bool | list[bool]:
    waiter = EventWaiter(events=events,
                         predicates=predicates,
                         timeout=timeout,
                         wait_for_all=wait_for_all, )

    success = waiter.wait()
    events_finished = waiter.events_finished
    if len(events_finished) == 1:
        events_finished = events_finished[0]
...
class EventLoop(metaclass=_SingletonMeta):
    """
    Scalable event dispatcher:
      - Indexes waiters by Event to make _event_set O(#waiters_watching_event)
      - Thread-safe via a re-entrant lock
      - Supports stale_event_time precheck & immediate notify
    """

    def __init__(self):
        self.events = weakref.WeakSet()  # avoid leaking Event instances
    ...
    def addEvent(self, event: Event):
        with self._waiters_lock:
            ...
            # Register the event set callback exactly once
    ...
    def addWaiter(self, waiter: EventWaiter):
        ...
        with self._waiters_lock:
            ...
            if waiter.stale_event_time is not None and waiter.stale_event_time > 0:
                # Ensure history long enough
                for ev in waiter.events:
        ...
        if to_notify:
            # convenience: for single-event waits
            if len(waiter.matched_payloads) == 1:
                _, d = waiter.matched_payloads[0]
                waiter.data = d
            waiter.notify()
    ...
    def removeWaiter(self, waiter: EventWaiter):
        """
        Remove waiter from global list and per-event index.
    ...
    def _event_set(self, data, event: Event, flags, *args, **kwargs):
        ...
        with self._waiters_lock:
            ...
            for waiter in watchers:
                ...
                for pos, ev in enumerate(waiter.events):
                    ...
                    if ok:
                        ...
                        if not waiter.events_finished[pos]:
                            ...
                            waiter.set_match(pos, flags, data)
                ...
                if satisfied:
                    ...
                    to_cleanup.append((waiter, tuple(waiter.events)))

            for waiter, evs in to_cleanup:
                for ev in evs:
                    s = self.waiters_by_event.get(ev)
                    if s is not None:
                        s.discard(waiter)
        ...
        for waiter in to_notify:
            if len(waiter.matched_payloads) == 1:
                _, d = waiter.matched_payloads[0]
                waiter.data = d  # convenience for single-event listeners
            waiter.notify()
