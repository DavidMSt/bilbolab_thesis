...
from extensions.control_gui.src.lib.widgets.sliders import SliderWidget
...
def create_child_gui(host, port, parent_port):
    ...
    button1 = Button(id='button1', text='CB1', color=[0, 0.6, 0])
    page1.addObject(button1)
    page2.addObject(button1)

    button1.callbacks.click.register(
        lambda *args, **kwargs: button1.update(color=random_color_from_palette('dark')))
    ...
    page1.addObject(slider2, width=6, height=2)
    page2.addObject(slider2, width=6, height=2)
    return child_app
...
def main():
    ...
    parent_app.addChildGUI(child_address=host, child_port=child_port,
                           parent_object=':parent_gui:/category1', )

    time.sleep(5)
    #
    category_new = child_app.addCategory(Category(id='category_new', name='New Cat'))
