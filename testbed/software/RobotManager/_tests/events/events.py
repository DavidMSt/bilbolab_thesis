...
class SubscriberMatch:
    ...
    def caused_by_group(self, group: Subscriber) -> bool:
        """
        True if this match involved `group` (useful for 'did the AND side win?' in OR trees).
...
class Subscriber:
    ...
    # ------------------------------------------------------------------------------------------------------------------
    def register_queue(self, q: queue.Queue) -> bool:
        """
        Register a persistent queue that will receive (data, match) tuples
        for every satisfied event. Returns False if the subscriber has
...
class SubscriberListener:
    ...
    def __init__(self,
        ...
        # Compound subscriber that listens to the real subscriber OR the stop event.
        ...
        # Persistent queue registered with the compound subscriber so that
        # we do not miss any events between waits.
        self._queue: queue.Queue = queue.Queue(maxsize=1)
        self._compound_subscriber.register_queue(self._queue)
    ...
    def _task(self):
        while not self._exit:
            try:
                item = self._queue.get(timeout=self._timeout) if self._timeout is not None else self._queue.get()
            except queue.Empty:
            ...
            # Sentinel from Subscriber.stop()
            if item is self._compound_subscriber._SENTINEL:
                self.logger.debug("Received sentinel from compound subscriber. Stopping.")
                break
            ...
            # Strip the stop event from the compound subscriber result
