from core.archive.device_manager import DeviceManager
from core.archive.device import Device
from core.utils.network.network import getHostIP
from robots.bilbo.manager.bilbo_manager_cli import BILBO_Manager_CommandSet
from core.utils.callbacks import callback_definition, CallbackContainer
from robots.bilbo.robot.bilbo import BILBO
from robots.bilbo.robot.bilbo_definitions import BILBO_Control_Mode, TWIPR_IDS, TWIPR_PASSWORD, \
    TWIPR_REMOTE_START_COMMAND, \
...
class BILBO_Manager:
    ...
    def __init__(self, host=None, robot_auto_start=True):
        """
        ...
        self.deviceManager = DeviceManager(address=host)
        ...
        # Logger
        self.logger = Logger('BILBO Manager')
    ...
    def close(self, *args, **kwargs):
        self.logger.info("Close BILBO Manager")
        if self.scanner is not None:
    ...
    def getRobotById(self, robot_id):
        ...
        if robot_id not in self.robots.keys():
            self.logger.warning(f"No robot with id {robot_id} is connected.")
            return None
    ...
    def _newDevice_callback(self, device: Device, *args, **kwargs):
        ...
        if not (device.information.device_class == 'robot' and device.information.device_type == 'bilbo'):
            ...
            if device.information.device_class == 'robot':
                self.logger.warning(f"Robot attempted to connect with type {device.information.device_type}")
            return
        ...
        if robot.device.information.device_id in self.robots.keys():
            self.logger.warning(f"New Robot connected, but ID {robot.device.information.device_id} is already in use")

        self.robots[robot.device.information.device_id] = robot

        self.cli_command_set.addChild(robot.interfaces.cli_command_set)

        self.logger.info(f"New Robot connected with ID: \"{robot.device.information.device_id}\"")
    ...
    def _deviceDisconnected_callback(self, device, *args, **kwargs):
        ...
        self.logger.warning(f"Robot {device.information.device_id} disconnected")
    ...
    def _scannerRobotFound_callback(self, name, ip_address, *args, **kwargs):
        self.logger.info(f"Scanner found robot {name} with IP address {ip_address}")
        self._startTWIPRRemote(name, ip_address)

    # ------------------------------------------------------------------------------------------------------------------
    def _scannerRobotLost_callback(self, name, ip_address, *args, **kwargs):
        self.logger.info(f"Scanner lost robot {name} with IP address {ip_address}")
