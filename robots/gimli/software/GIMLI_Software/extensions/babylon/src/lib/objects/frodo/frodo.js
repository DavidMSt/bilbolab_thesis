...
export class BabylonFrodo extends BabylonObject {
    ...
    constructor(id, scene, payload = {}) {
        ...
        // Create a line from the origin [0,0,0] to Frodo
        const lineToFrodo = new BabylonObjectLabeledLineDrawing("line1", scene, {
            config: {
                start: [0, 0, 0],          // fixed world point
                end: this,                // follow Frodo's root transform
                lineColor: [0, 0, 0, 0.4],
                lineWidth: 0.005,
                lineStyle: "dotted"
            }
    ...
    onMeshLoaded(newMeshes) {
        ...
        // Static correction rotation applied ONLY to the model (not to the world/root)
    ...
    _sampleVisibilityPointsAdaptive() {
        ...
        const radius = cfg.fov_radius * cfg.scaling;
        const fovDeg = cfg.fov_angle_deg;
        ...
        const castSweep = (angles) => {
            ...
            for (let ia = 0; ia < angles.length; ia++) {
                ...
                const displayY = originBaseY + 0.001;
        ...
        const centerY = (typeof cfg.groundY === "number" ? cfg.groundY : 0) + 0.001;
    ...
    _buildOrUpdateFovMeshes(points, distances) {
        ...
        const center = new Vector3(centerPos.x, (this.config.groundY ?? 0) + 0.001, centerPos.z);
    ...
    // Apply DYNAMIC pose to the ROOT; keep static correction on modelRoot
