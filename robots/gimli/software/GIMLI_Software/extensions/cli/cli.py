...
class Command:
    ...
    def _parseCommandString(self, command_string: str | None) -> dict | None:
        """
        Tokenize the input, pull off --long and -s flags (with optional 0/1), then
        collect raw_positionals.  If there’s exactly one extra raw positional *and*
        exactly one unset flag, and that positional is '0' or '1', consume it.
        """
        ...
        while i < L:
            ...
            if tok.startswith('--'):
                ...
                # flag with optional explicit value
                if arg.is_flag:
                    # check next token for '0' or '1'
                ...
                # non-flag consumes exactly one value (or bracketed list)
                    ...
                    if arg.array_size > 0:
                        ...
                        if len(parts) != arg.array_size:
                            self.logger.error(
                                f"Argument {name} expects a list of {arg.array_size} values."
                            )
                            return None
            ...
            # --- SHORT FLAG ---
                ...
                if arg.is_flag:
                    ...
                    if arg.array_size > 0:
                        ...
                        if len(parts) != arg.array_size:
                            self.logger.error(
                                f"Argument {arg.name} expects a list of {arg.array_size} values."
                            )
                            return None
        ...
        if len(raw_positionals) > max_pos:
            ...
            # if exactly one extra and exactly one unset flag, and it's '0'/'1', consume it
            ...
            if extra == 1 and len(unset_flags) == 1 and re.fullmatch(r'[01]', candidate):
                ...
                self.logger.error(
                    ...
                    f"Expected at most {max_pos} but got {len(raw_positionals)}."
                )
                return None
