...
def coerce_to_type(value: Any, target_type: Any, config: Optional[Config]) -> Any:
    """
    Convert 'value' into 'target_type' using typing annotations.
    This is the generic engine that replaces ad-hoc type_hooks.
    """
    if value is None:
        return None
    ...
    if _is_dataclass_type(target_type):
        # recurse using your existing from_dict (this function lives in same module)
    ...
    if origin in (list, List, Sequence, tuple, Tuple, set, frozenset):
        ...
        def coerce_elem(e, i):
            # For homogeneous containers like List[T]/Set[T]
            if origin in (list, List, Sequence, set, frozenset):
                t = elem_types[0] if elem_types else Any
                return coerce_to_type(e, t, config) if t is not Any else e
            # For fixed-length tuples Tuple[T1,T2,...] or variadic Tuple[T, ...]
            if origin in (tuple, Tuple):
                if len(elem_types) == 2 and elem_types[1] is Ellipsis:
                    return coerce_to_type(e, elem_types[0], config)
                if i >= len(elem_types):
                    raise TypeError(f"Tuple index {i} out of bounds for {target_type}")
                return coerce_to_type(e, elem_types[i], config)
            return e
    ...
    # ----- Fall back: dataclass types hidden behind typing e.g. Annotated -----
    if isinstance(target_type, type):
        # Basic scalar casts (int/float/str/bool)
        try:
            return _simple_cast(value, target_type)
        except Exception:
            pass
        # As a last resort, if the annotated type is itself a dataclass or enum
        if _is_dataclass_type(target_type):
            return from_dict_auto(target_type, value, config=config)
        if _is_enum_type(target_type):
            return _coerce_enum(value, target_type)

    # If nothing matched, return as-is (dacite's check_types will catch real mismatches)
    return value
