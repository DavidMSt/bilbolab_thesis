...
/* Register Entries */
/* Firmware */
core_utils_RegisterEntry<uint32_t, void> regentry_read_firmware_tick(
		&register_map, REG_ADDRESS_R_FIRMWARE_TICK, &frodo_firmware.tick);

/* Control */
core_utils_RegisterEntry<void, motor_input_t> regentry_function_set_speed(
		&register_map, REG_ADDRESS_F_SET_SPEED, &frodo_firmware.drive,
		&FRODO_Drive::setSpeed);

/* Utils */
core_utils_RegisterEntry<void, rgb_color_struct_t> regentry_function_setExternalLED(
		&register_map, REG_ADDRESS_F_EXTERNAL_LED, &extender,
		&RobotControl_Extender::rgbLEDStrip_extern_setColor);
core_utils_RegisterEntry<void, buzzer_beep_struct_t> regentry_f_firmware_beep(
		&register_map, REG_ADDRESS_F_FIRMWARE_BEEP, &rc_buzzer,
		&RobotControl_Buzzer::beep);
 ...
 */
void firmware() {
	osThreadNew(start_firmware_task, (void*) &frodo_firmware,
...
void FRODO_Firmware::init() {
	...
	frodo_logging_config_t logging_config = { .firmware = this, .drive =
			&this->drive, .use_buffer = false };

	this->logging.init(logging_config);

	// Initialize the speed controller
	frodo_drive_config_t drive_config = { .motor_left_dir_port =
	MOTOR_LEFT_DIR_PORT, .motor_left_dir_pin = MOTOR_LEFT_DIR_PIN,
			.motor_left_htim = MOTOR_LEFT_PWM_TIMER, .motor_left_timer_channel =
			MOTOR_LEFT_PWM_CHANNEL, .motor_left_encoder_htim =
			MOTOR_LEFT_ENCODER_TIMER, .motor_left_direction =
			MOTOR_LEFT_DIRECTION, .motor_left_velocity_scale = 1,

			.motor_right_dir_port = MOTOR_RIGHT_DIR_PORT, .motor_right_dir_pin =
			MOTOR_RIGHT_DIR_PIN, .motor_right_htim =
			MOTOR_RIGHT_PWM_TIMER, .motor_right_timer_channel =
			MOTOR_RIGHT_PWM_CHANNEL, .motor_right_encoder_htim =
			MOTOR_RIGHT_ENCODER_TIMER, .motor_right_direction =
			MOTOR_RIGHT_DIRECTION, .motor_right_velocity_scale = 1,
...
void FRODO_Firmware::controlTask() {
	...
	while (true) {
		...
		HAL_GPIO_TogglePin(FRODO_CONTROL_TASK_TICK_PORT,
		FRODO_CONTROL_TASK_TICK_PIN);
		...
		osDelayUntil(
				global_tick
						+ (uint32_t) (1000.0
								/ (float) FRODO_CONTROL_TASK_FREQUENCY));
	}
}

/* ---------------------------------------------------------------------------- */
frodo_general_sample_t FRODO_Firmware::getSample() {
	frodo_general_sample_t sample = { .tick = this->tick, .state = 1,
			.update_time = FRODO_CONTROL_TASK_TIME_MS / 1000.0,
			.battery_voltage = extender.getBatteryVoltage() };
